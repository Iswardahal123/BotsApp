...
module.exports = {
    ...
    async handle(client: Client, chat: proto.IWebMessageInfo, BotsApp: BotsApp, args: string[], commandHandler: Map<string, Command>): Promise<void> {
        try {
            ...
            client.sendMessage(BotsApp.chatId, HELP.COMMAND_INTERFACE + format(HELP.ERROR_MSG, prefix[0]), MessageType.text).catch(err => inputSanitization.handleError(err, client, BotsApp));
        } catch (err) {
