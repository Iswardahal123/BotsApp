...
module.exports = {
    ...
    async handle(client: Client, chat: proto.IWebMessageInfo, BotsApp: BotsApp, args: string[], commandHandler: Map<string, Command>): Promise<void> {
        try {
            const prefix = config.PREFIX.match(/\[(.*)\]/)?.[1] || config.PREFIX.replace('^', '');
            const primaryPrefix = prefix[0] || '.';

            let helpMessage: string;
            if(!args[0]){
                helpMessage = format(HELP.HEAD, commandHandler.size.toString(), primaryPrefix);

                const sortedCommands = Array.from(commandHandler.values()).sort((a, b) => a.name.localeCompare(b.name));

                sortedCommands.forEach(element => {
                    helpMessage += format(HELP.TEMPLATE, primaryPrefix + element.name, element.description);
                });
            ...
            client.sendMessage(BotsApp.chatId, HELP.COMMAND_INTERFACE + format(HELP.ERROR_MSG, primaryPrefix), MessageType.text).catch(err => inputSanitization.handleError(err, client, BotsApp));
        } catch (err) {
