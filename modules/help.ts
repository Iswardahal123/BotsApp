...
module.exports = {
    ...
    async handle(client: Client, chat: proto.IWebMessageInfo, BotsApp: BotsApp, args: string[], commandHandler: Map<string, Command>): Promise<void> {
        try {
            ...
            client.sendMessage(BotsApp.chatId, format(HELP.ERROR_MSG, commandHandler.size.toString()), MessageType.text).catch(err => inputSanitization.handleError(err, client, BotsApp));
        } catch (err) {
