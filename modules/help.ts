        ...
        try {
            ...
            if(command){
                ...
                prefixes.split("").forEach(prefix => {
                    triggers.push(prefix + command.name);
                });
                var triggersStr = triggers.join(", ");

                if(command.demo?.isEnabled) {
                    var buttons: proto.Message.ButtonsMessage.IButton[] = [];
                    helpMessage += format(HELP.COMMAND_INTERFACE_TEMPLATE, triggersStr, command.extendedDescription) + HELP.FOOTER;
                ...
                helpMessage += format(HELP.COMMAND_INTERFACE_TEMPLATE, triggersStr, command.extendedDescription);
                client.sendMessage(BotsApp.chatId, helpMessage, MessageType.text).catch(err => inputSanitization.handleError(err, client, BotsApp));
                return;
            }
            client.sendMessage(BotsApp.chatId, HELP.COMMAND_INTERFACE + "```Invalid Command. Check the correct name from```  *" + prefixes[0] + "help*  ```command list.```", MessageType.text).catch(err => inputSanitization.handleError(err, client, BotsApp));
        } catch (err) {
