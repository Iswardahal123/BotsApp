...
module.exports = {
    ...
    async handle(client: Client, chat: proto.IWebMessageInfo, BotsApp: BotsApp, args: string[], commandHandler: Map<string, Command>): Promise<void> {
        try {
            var prefixes: string = config.PREFIXES;
            let helpMessage: string;
            if(!args[0]){
                helpMessage = format(HELP.HEAD, {count: commandHandler.size.toString()});
                const sortedCommands = Array.from(commandHandler.values()).sort((a, b) => a.name.localeCompare(b.name));
                sortedCommands.forEach(element => {
                    helpMessage += format(HELP.TEMPLATE, {command: prefixes[0] + element.name, info: element.description});
                });
            ...
            client.sendMessage(BotsApp.chatId, HELP.COMMAND_INTERFACE + format(Strings.general.ERROR_MSG, {prefix: prefixes[0]}), MessageType.text).catch(err => inputSanitization.handleError(err, client, BotsApp));
        } catch (err) {
